...
public class MovieServiceMock implements MovieService {
	...
	public void saveMovie(Movie movie) {
		...
		MovieException be = new MovieException();
		...
		movie.setId(lstMovies.size()+1);
		System.out.println("ajout 4 :"+ movie.getId());
		lstMovies.add(movie);
		System.out.println("ajout 5");
	}

	private void validateTitle(String data, MovieException be) {
		if (data == null || data.isBlank()) {
			be.addError("Le titre est obligatoire");
		}
	}

	private void validateGenre(Genre data, MovieException be) {
		if (data == null || data.getId() == 0) {
			be.addError("Le genre est obligatoire");
		}
	}

	private void validateDirector(Participant data, MovieException be) {
		if (data == null || data.getId() == 0) {
			be.addError("Le réalisateur est obligatoire");
		}
	}

	private void validateYear(int data, MovieException be) {
		if (data < 1900 || data > 2100) {
			be.addError("L'année n'est pas correcte");
		}
	}

	private void validateDuration(int data, MovieException be) {
		if (data <= 0) {
			be.addError("La durée est positive");
		}
	}

	private void validateSynopsis(String data, MovieException be) {
	...
	private void validateNote(int data, MovieException be) {
		if (data < 0 || data > 5) {
			be.addError("La note doit être entre 0 et 5");
		}
	}

	private void validateComments(String data, MovieException be) {
	...
	private void validateMember(Member data, MovieException be) {
		if (data == null || data.getId() == 0) {
